...
class ContasPagarService extends BaseService implements IService
{
    ...
    public function index($request, $paginacao)
    {
        return $this->handleTryCatch(function () use ($request, $paginacao) {
            $lancamento = $this->getDefaultRepository()
                ->pushCriteria(new ContasPagarCriteria($request))
                ->pushCriteria(new OrderCriteria($request))
                ->skipPresenter(true)
                ->paginate($paginacao);
            return self::transformerData($lancamento, ContasPagarPresenter::class);
        }, false);
    }
    ...
    public function create(array $data): mixed
    {
        return $this->handleTryCatch(function () use ($data) {
            ...
            $data['tipo'] = TipoLancamento::SAIDA;
            $data['origem_lancamento'] = TipoLancamento::SAIDA;
            ...
            if ($data['num_parcela'] > 1) {
                ...
                $retorno = $this->lancamentofinanceiroService->parcelar(
                    ['lancamento_id' => $retorno->id, 'content' => $data]
                );
            }
            if ($data['recorrente'] == true) {
                $retorno = $this->lancamentofinanceiroService->recorrencia($retorno->id);
            };
            ...
            return self::transformerData($retorno, ContasPagarPresenter::class);
        }, true);
    }
    ...
    public function baixaManual(array $data)
    {
        return $this->handleTryCatch(function () use ($data) {
            ...
            if ($lancamento->recorrente === true) {
                $recorrencia = $lancamento->toArray();
                if ($data['mudar_data_recorrencia'] != "false") {
                    $recorrencia['data_vencimento'] =  $data['data_vencimento_recorrencia'];
                    $lancamentoNovo = LancamentoFinanceiro::create($recorrencia);
                } else {
                    $mes = Carbon::now()->format('m');
                    $data_vencimento = Carbon::parse($lancamento->data_vencimento)->setMonth($mes + 1);
                    $recorrencia['data_vencimento'] =  $data_vencimento;
                    $lancamentoNovo = LancamentoFinanceiro::create($recorrencia);
                }
            }
            ...
            return self::transformerData($lancamentoNovo, ContasPagarPresenter::class);
        }, true);
    }
    ...
    public function agrupar(array $request)
    {
        return $this->handleTryCatch(function () use ($request) {
            ...
            $request['origem_lancamento'] = TipoLancamento::SAIDA;
            ...
            return transformer_data($newLancamento, ContasPagarPresenter::class);
        }, true);
    }
    ...
    {
        return $this->handleTryCatch(function () use ($request) {
            $lancamentos = $this->getDefaultRepository()->skipPresenter(true)->find($request['id']);
    ...
    public function getFilhos($id, $paginacao, $request)
    {
        return $this->handleTryCatch(function () use ($id, $paginacao, $request) {
            ...
            return transformer_data($lancamentos, ContasPagarPresenter::class);
        }, true);
    }
    ...
    public function update(array $data, $id): mixed
    {
        return $this->handleTryCatch(function () use ($data, $id) {
            ...
            $lancamentos->save();
            return self::transformerData($lancamentos, ContasPagarPresenter::class);
        }, true);
    }
    public function parcelaAvulsa($request): mixed
    {
        return $this->handleTryCatch(function () use ($request) {
            ...
            $newLancamento->numero_documento = isset($request['numero_documento']) ? $request['numero_documento'] : $numeroGerado;
            $newLancamento->save();
